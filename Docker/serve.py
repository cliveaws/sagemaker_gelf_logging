...
log.addHandler(GelfUdpHandler(host=os.environ['GELF_LOGGING_HOST'], port=12201))
...
class ServiceManager(object):
    ...
    def _redirect_to_gelf(self, pid):
        log.info(f'redirecting PID {pid}')
        subprocess.run(['reredirect', '-m', '/sagemaker/gelf_fifo', f'{pid}'])
